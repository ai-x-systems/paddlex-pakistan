...
// Perforation holes scattered across the paddle face — matches the pattern
// baked into the 3D centerpiece's canvas texture, so both paths look the same.
...
const entrance = {
  hidden: { opacity: 0, x: 60, y: -20, scale: 0.85 },
  show: {
    opacity: 1,
    x: 0,
    y: 0,
...
// Continuous slow spin around the Y axis — mirrors the 3D centerpiece's
// auto-rotate so the fallback reads the same even without WebGL.
const spin = {
  rotateY: [0, 360],
...
export function HeroRacketBall() {
  ...
  const ballShiftX = useTransform(springX, [-1, 1], [-8, 8]);
  const ballShiftY = useTransform(springY, [-1, 1], [-6, 6]);
  ...
  return (
    <div ref={wrapRef} className="pointer-events-none absolute inset-0 overflow-visible" style={{ perspective: 900 }}>
      ...
      <motion.div animate={idleFloat} className="absolute right-0 top-0" style={{ transformStyle: "preserve-3d" }}>
        {/* entrance settle */}
        <motion.div variants={entrance} initial="hidden" animate="show" style={{ transformStyle: "preserve-3d" }}>
          {/* continuous spin, independent of the hover tilt below */}
          <motion.div animate={reducedMotionRef.current ? undefined : spin} style={{ transformStyle: "preserve-3d" }}>
            {/* cursor tilt (rotateX only, so it never fights the spin) */}
        ...
        >
          <svg width="34" height="34" viewBox="0 0 34 34" style={{ filter: "drop-shadow(0 6px 10px rgba(0,0,0,0.5))" }}>
            ...
            <path d="M4 12C9 15 14 15 17 12C20 15 25 15 30 12" stroke="#0a0a0a" strokeWidth="1" opacity="0.3" fill="none" />
            <path d="M4 22C9 19 14 19 17 22C20 19 25 19 30 22" stroke="#0a0a0a" strokeWidth="1" opacity="0.3" fill="none" />
          </svg>
        </motion.div>
      </motion.div>
    </div>
